...
import useWebAnimations, { lightSpeedInRight,lightSpeedInLeft,rotateIn} from "@wellyshen/use-web-animations";
import './App.css';

function App() {
  
  const { ref:skyRef } = useWebAnimations({...lightSpeedInRight});
  const { ref:earthRef } = useWebAnimations({...lightSpeedInLeft});
  const { ref:redQueenAliceRef } = useWebAnimations({...rotateIn});
